...
class Subforum():
    ...
    def _unzip_and_load(self, zipped_catfile):
        ...
        if os.path.exists(questionfile) and os.path.exists(
    ...
    def get_posts_dups_nodups_and_related(self):
        ...
        related = []
        for p in self.postdict:
            if len(self.postdict[p]['dups']) == 0 and len(
                    self.postdict[p]['related']) == 0:
                nodups.append(p)
            if len(self.postdict[p]['related']) > 0:
                related.append(p)
            if len(self.postdict[p]['dups']) > 0:
    ...
    def get_ordered_list_of_posts(self):
        ...
        return sorted(
            [(i, datetime.datetime.strptime(self.get_postdate(i), '%Y-%m-%d'))
             for i in self.postdict], key=lambda x: x[1], reverse=True)
    ...
    def get_postdate(self, postid):
        ...
        return str(cdate.year) + '-' + cdate.strftime(
            '%m') + '-' + cdate.strftime('%d')
    ...
    def get_posttime(self, postid):
        ...
        cdate = datetime.datetime.strptime(
            self.postdict[postid]['creationdate'], "%Y-%m-%dT%H:%M:%S.%f")
        return cdate.strftime('%H') + ':' + cdate.strftime(
            '%M') + ':' + cdate.strftime('%S')
    ...
    def get_acceptedanswer_date(self, answerid):
        ...
        if self.answerdict[answerid]['acceptedanswerdate'] == 0:
            ...
            adate = datetime.datetime.strptime(
                self.answerdict[answerid]['acceptedanswerdate'],
                "%Y-%m-%dT%H:%M:%S.%f")
            return str(adate.year) + '-' + adate.strftime(
                '%m') + '-' + adate.strftime('%d')
    ...
    def get_commentdate(self, commentid):
        ...
        cdate = datetime.datetime.strptime(
            self.commentdict[commentid]['creationdate'],
            "%Y-%m-%dT%H:%M:%S.%f")
        return str(cdate.year) + '-' + cdate.strftime(
            '%m') + '-' + cdate.strftime('%d')
    ...
    def change_to_default_stopwords(self, stopwordset='middle'):
        ...
        if stopwordset == 'nltk':
            if self.__nltk_stopwords != []:
                ...
                self.__stopwords = [u'i', u'me', u'my', u'myself', u'we',
                                    u'our', u'ours', u'ourselves', u'you',
                                    u'your', u'yours', u'yourself',
                                    u'yourselves', u'he', u'him', u'his',
                                    u'himself', u'she', u'her', u'hers',
                                    u'herself', u'it', u'its', u'itself',
                                    u'they', u'them', u'their', u'theirs',
                                    u'themselves', u'what', u'which', u'who',
                                    u'whom', u'this', u'that', u'these',
                                    u'those', u'am', u'is', u'are', u'was',
                                    u'were', u'be', u'been', u'being', u'have',
                                    u'has', u'had', u'having', u'do', u'does',
                                    u'did', u'doing', u'a', u'an', u'the',
                                    u'and', u'but', u'if', u'or', u'because',
                                    u'as', u'until', u'while', u'of', u'at',
                                    u'by', u'for', u'with', u'about',
                                    u'against', u'between', u'into',
                                    u'through', u'during', u'before', u'after',
                                    u'above', u'below', u'to', u'from', u'up',
                                    u'down', u'in', u'out', u'on', u'off',
                                    u'over', u'under', u'again', u'further',
                                    u'then', u'once', u'here', u'there',
    ...
    def perform_cleaning(self, s, maxcodelength=150, remove_stopwords=False,
                         remove_punct=False, stem=False):
    ...
    def _deal_with_code(self, s, maxcodelength):
        ...
        codes = re.findall(codepat,
                           s)  # re.M for multiline matching not necessary?
        n = 0
        newcodes = []
        for c in codes:
    ...
    def strip_tags(self,
                   html):  # Source: http://stackoverflow.com/questions/753052/strip-html-from-strings-in-python
        s = MLStripper()
        s.feed(html)
        return s.get_data()
    ...
    def _remove_tags(self, s):
        ...
        s = re.sub(
            r"<a href=\"https?://[a-z]+\.stackexchange\.com[^\"]+\">([^<]+)</a>",
            r"\1", s)
        ...
        s = re.sub(r"https?://([a-z]+\.)?stackexchange\.com[^ ]+",
                   "stackexchange-url", s)
    ...
    def _general_cleaning(self, s, codes, remove_punct=False):
        ...
        if remove_punct:
            ...
            if l:
                for m in l:
                    newbit = re.sub(r' ', '',
                                    m.group())  # Get rid of white space in abbreviations
                    newabbr = re.sub(r'\.', 'PPPP',
                                     newbit)  # change dots in abbreviations into 'PPPP'
                    s = re.sub(m.group() + ' +', ' ' + newabbr + ' ',
                               s)  # protect abbreviations
                    s = re.sub(r'\.', '',
                               s)  # remove all points that are not in abbreviations
                    s = re.sub(newabbr, newbit,
                               s)  # place dots back in abbreviations
            else:
                s = re.sub(r'\.', ' ', s)
            s = re.sub(r',', ' ', s)
            s = re.sub(r'\?', ' ', s)
        ...
        for i, u in newurllist:
            s = re.sub('GGGG' + str(i), u,
                       s)  # Escaping u here leads to backslashes in URLs.
    ...
    def split_for_classification(self, outputdir='.'):
        ...
        withdups = []
        withoutdups = []
        testfile = open(outputdir + '/' + self.cat + '_testpairs_large.txt',
                        'w')
        for i, postid in enumerate(testposts):
            dups = self.get_duplicates(postid)
            combine_with = testposts[
                           i + 1:]  # combine each post with older posts.
        ...
        while nrofnondups_forsmallset > 0:
            pos = randrange(len(
                withoutdups))  # randrange very conveniently excludes the last element of the list.
            picked_nondup = withoutdups[pos]
            withoutdups[pos] = withoutdups[
                -1]  # Replace picked element with last one in the list
            del withoutdups[-1]  # Cheap removal of element
            withoutdups_for_smallset.append(picked_nondup)
            nrofnondups_forsmallset -= 1

        testfile = open(outputdir + '/' + self.cat + '_testpairs_small.txt',
                        'w')
        for tup in withdups:
        ...
        print
        "total train pairs:", totaltrainpairs

        trainfile1 = open(outputdir + '/' + self.cat + '_trainpairs_01.txt',
                          'w')
        trainfile2 = open(outputdir + '/' + self.cat + '_trainpairs_02.txt',
                          'w')
        trainfile3 = open(outputdir + '/' + self.cat + '_trainpairs_03.txt',
                          'w')
        trainfile4 = open(outputdir + '/' + self.cat + '_trainpairs_04.txt',
                          'w')
        trainfile5 = open(outputdir + '/' + self.cat + '_trainpairs_05.txt',
                          'w')
        trainfile6 = open(outputdir + '/' + self.cat + '_trainpairs_06.txt',
                          'w')
        trainfile7 = open(outputdir + '/' + self.cat + '_trainpairs_07.txt',
                          'w')
        trainfile8 = open(outputdir + '/' + self.cat + '_trainpairs_08.txt',
                          'w')
        trainfile9 = open(outputdir + '/' + self.cat + '_trainpairs_09.txt',
                          'w')
        trainfile10 = open(outputdir + '/' + self.cat + '_trainpairs_10.txt',
                           'w')
    ...
    def _find_cutoff_date(self):
        ...
        for thres in datelist:
            if t[thres]['test'] >= 200:
                if t[thres]['train'] >= 4 * t[thres]['test']:
                    datum = str(thres.year) + '-' + str(
                        thres.month) + '-' + str(thres.day)
                    allcandidates.append((datum, t[thres]['test'],
                                          t[thres]['train'], t[thres]['lost']))
        ...
        position = len(
            allcandidates) / 2  # The best candiDATE is the one in the middle.
        if allcandidates != []:
    ...
    def average_ndcg_at(self, scorefile, cutoff=None,
            ...
            if relevantdocs != []:  # or (relateddocs != [] and include_related_posts): # We only compute the score for queries that actually do have duplicates in the indexed set.
                ...
                if include_related_posts:
                    relevance_judgements = map(
                        lambda x: 1.0 if x in relevantdocs else (
                ...
                I = self._get_I(cutoff, include_related_posts, relevantdocs,
                                relateddocs)
    ...
    def average_recall_at(self, scorefile, cutoff=None,
                          include_related_posts=False):
    ...
    def average_precision_at(self, scorefile, cutoff=None,
                             include_related_posts=False):
    ...
    def truncated_mean_average_precision(self, scorefile,
                                         include_related_posts=False):
    ...
    def evaluate_classification(self, scorefile):
        ...
        precision = self._compute_precision(truenegatives, truepositives,
                                            falsenegatives, falsepositives)
        recall = self._compute_recall(truenegatives, truepositives,
                                      falsenegatives, falsepositives)
        fscore = self._compute_fscore(recall, precision)
        accuracy = self._compute_accuracy(truenegatives, truepositives,
                                          falsenegatives, falsepositives)
        prec_pos, prec_neg = self._compute_precision_oneclass(truenegatives,
                                                              truepositives,
                                                              falsenegatives,
                                                              falsepositives)
        rec_pos, rec_neg = self._compute_recall_oneclass(truenegatives,
                                                         truepositives,
                                                         falsenegatives,
                                                         falsepositives)
        return {'precision': precision, 'recall': recall, 'fscore': fscore,
                'accuracy': accuracy, 'precision_positive_class': prec_pos,
                'precision_negative_class': prec_neg,
                'recall_positive_class': rec_pos,
                'recall_negative_class': rec_neg}
    ...
    def _compute_precision(self, truenegatives, truepositives, falsenegatives,
                           falsepositives):
    ...
    def _compute_recall(self, truenegatives, truepositives, falsenegatives,
                        falsepositives):
    ...
    def _compute_accuracy(self, truenegatives, truepositives, falsenegatives,
                          falsepositives):
        ''' Takes the nr of truenegatives, truepositives, falsenegatives, and falsepositives as input and returns the accuracy. '''
        return (truepositives + truenegatives) / float(
            truepositives + falsepositives + truenegatives + falsenegatives)

    def _compute_precision_oneclass(self, truenegatives, truepositives,
                                    falsenegatives, falsepositives):
    ...
    def _compute_recall_oneclass(self, truenegatives, truepositives,
                                 falsenegatives, falsepositives):
